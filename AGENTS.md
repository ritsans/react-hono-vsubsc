...
上の「シンプルさ優先」はコードの構造に対するルールであり、**UI の見た目には適用しない**。UI はシンプルさよりリッチさを優先する。

- DaisyUI の標準部品を並べただけで済ませず、配色・余白・タイポグラフィ・アイコン・状態表示（読み込み中・空・エラー）まで作り込む
- UI を実装・変更するときは、利用できるデザイン用スキル（`frontend-design` / `hallmark` など）を必ず使う
- 見た目をリッチにしても、コード側は上のルール（一本道・先回りした共通化をしない）に従う
...
- `pnpm check` / `pnpm build` はいずれも `tsc -b` を使うので安全
...
- `README.md` / `AGENTS.md` / `CLAUDE.md` / `pnpm-workspace.yaml` / `pnpm-lock.yaml` は Biome が扱わない形式なので、除外指定は不要
...
※ ドライバ・スキーマ（`src/server/db/schema.ts`、`user` / `subscription`）・接続コード（`src/server/db/client.ts`）・マイグレーションは導入済み。変更・実装する際は以下に従うこと。
...
コード変更後は以下を順に実行し、失敗した時点で止めて原因を確認する（`pnpm check` 単体には lint が含まれない）。

```bash
pnpm lint
pnpm check   # tsc -b → vite build → wrangler deploy --dry-run
```

テストは別途 `pnpm test`。`wrangler deploy --dry-run` は **実際のデプロイを行わず**、バンドルと設定の検証のみ。
